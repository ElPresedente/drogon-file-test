...
class test_root : public drogon::HttpController<test_root>
{
  public:
    ...
    ADD_METHOD_VIA_REGEX(test_root::handler,"/test_root/save/(.*)",Get);
    ADD_METHOD_VIA_REGEX(test_root::file_handler,"/test_root/save/(.*)",Post);

    ADD_METHOD_VIA_REGEX(test_root::send_file, "/test_root/get/(.*)", Get);
    METHOD_LIST_END
    ...
    void send_file(const HttpRequestPtr& req, std::function<void (const HttpResponsePtr &)> &&callback, std::string p1) const;
};
